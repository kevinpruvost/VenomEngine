...
Error VulkanSurface::CreateSurface(VkInstance instance, Context* context)
{
#if defined(_WIN32)
    ...
    if (auto res = vkCreateWin32SurfaceKHR(instance, &createInfo, nullptr, &surface); res != VK_SUCCESS) {
    ...
    if (auto res = vkCreateMacOSSurfaceMVK(instance, &createInfo, nullptr, &surface); res != VK_SUCCESS) {
    ...
    if (auto res = vkCreateXlibSurfaceKHR(instance, &createInfo, nullptr, &surface); res != VK_SUCCESS) {
    ...
    if (auto res = vkCreateWaylandSurfaceKHR(instance, &createInfo, nullptr, &surface); res != VK_SUCCESS) {
