...
vc::Error Surface::CreateSurface(vc::Context* context)
{
    ...
    if (auto res = vkCreateWin32SurfaceKHR(Instance::GetVkInstance(), &createInfo, nullptr, &surface); res != VK_SUCCESS) {
    ...
    if (auto res = vkCreateMacOSSurfaceMVK(VulkanInstance::GetVkInstance(), &createInfo, nullptr, &surface); res != VK_SUCCESS) {
    ...
    if (auto res = vkCreateXlibSurfaceKHR(VulkanInstance::GetVkInstance(), &createInfo, nullptr, &surface); res != VK_SUCCESS) {
    ...
    if (auto res = vkCreateWaylandSurfaceKHR(VulkanInstance::GetVkInstance(), &createInfo, nullptr, &surface); res != VK_SUCCESS) {
